...
def create_hourly_orders(df):
    orders_per_hour = df.groupby("order_hour")["order_id"].count()
    orders_per_hour.index = orders_per_hour.index.map(
        lambda x: f"{x % 12 or 12} {'AM' if x < 12 else 'PM'}"
    )
    return orders_per_hour.sort_values(ascending=False).reset_index()


def create_orders_per_day(df):
    return (
...
def create_orders_per_month(df):
    return (
...
with st.sidebar:
    ...
    st.image("dashboard/assets/ecommerce-logo.png")
...
if main_df.empty:
    ...
    hourly_orders_df = create_hourly_orders(main_df)
    orders_per_day_df = create_orders_per_day(main_df)
    orders_per_month_df = create_orders_per_month(main_df)
    ...
    with profit1:
        ...
        base = alt.Chart(
            ...
            y=alt.Y("product_category_name_english:O", title="Product Category").sort(
                "-x"
            ),
    ...
    with profit2:
        ...
        bars = base.mark_bar().encode(
            color=alt.Color(
                "profit_margin:Q", scale=alt.Scale(scheme="reds"), legend=None
            ),
        )
    ...
    with order2:
        ...
        bars = base.mark_bar().encode(
            color=alt.Color(
                "order_id:Q", scale=alt.Scale(scheme="greens"), legend=None
            ),
        )
